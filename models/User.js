...
userSchema.statics.authMiddleware = function(req, res, next) {
  var token = req.cookies.foodJcookie;
  try {
    var payload = jwt.decode(token, JWT_SECRET);
  } catch(err) {
    return res.clearCookie('lilcookie').status(401).send();
  }
  // we have a valid token

  User.findById(payload.userId).select({password: 0})
  .populate('rests')
  .exec(function(err, user) {
    if(err || !user) {
      return res.clearCookie('lilcookie').status(401).send(err);
    }
    // the user exists!
